...
from reflekt.avo.parser import (
    parse_avo_event,
    parse_avo_property,
)
...
class AvoPlan(object):
    ...
    def build_reflekt(self, plan_dir):
        ...
        traits_json = (
            self._json_obj.get("rules").get("identify", {}).get("properties", {}).get("traits", {}).get("properties")
        )
        ...
        group_traits_json = (
            self._json_obj.get("rules").get("group", {}).get("properties", {}).get("traits", {}).get("properties")
        )
    ...
    def _build_reflekt_plan_file(self, plan_dir):
        ...
        plan_obj = {"name": self.display_name}  # avo refers to the plan name as the `display_name` in their API
    ...
    def _build_reflekt_event_file(self, events_dir, event_json):
        ...
        event_file_name = dasherize(underscore(event_json["name"].replace(" ", "").replace("/", "")))
    ...
    def _build_reflekt_identify_file(self, plan_dir, traits_json):
        traits = [parse_avo_property(name, prop_json) for (name, prop_json) in traits_json.items()]
    ...
    def _build_reflekt_group_file(self, plan_dir, group_traits_json):
        traits = [parse_avo_property(name, prop_json) for (name, prop_json) in group_traits_json.items()]
